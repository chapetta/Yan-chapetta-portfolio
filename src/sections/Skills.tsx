...
import { hardSkills, skills } from '../utils/content'
              ...
              <ul className="mt-5 space-y-3 text-sm text-slate-300">
                {categories.map((category) => (
                  <li key={category} className="flex items-start gap-3">
                    ...
                    <span>
                      <strong className="block font-semibold text-white">{category}</strong>
                      {skills
              ...
              <ul className="mt-5 space-y-3 text-sm text-slate-300">
                {hardSkills.map((skill) => (
                  <li key={skill.name} className="flex items-start gap-3">
                    ...
                    <span>
                      <strong className="block font-semibold text-white">{skill.name}</strong>
                      <span className="text-xs uppercase tracking-[0.35em] text-slate-400">
                        {skill.level}
                      </span>
                      <span className="mt-1 block leading-snug text-slate-300">{skill.description}</span>
                    </span>
                  </li>
                ))}
              </ul>
