...
export function SkillCard({ skill, index }: SkillCardProps) {
  return (
    <motion.div
      ...
      className={clsx(
        'group flex flex-col gap-3 rounded-2xl border border-white/10 bg-slate-900/60 p-4 transition-all duration-500 hover:-translate-y-1 hover:border-primary/60 hover:bg-primary/15',
        skill.highlight && 'border-primary/70 bg-primary/20 shadow-glow-primary'
      )}
      aria-label={`Habilidade ${skill.name} nível ${skill.level}`}
      title={`${skill.name} • ${skill.level}`}
    >
      <div className="flex items-center gap-3">
        <span className="relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-xl bg-slate-950/70 transition-colors">
          <img
            src={skill.iconUrl}
            alt={skill.name}
